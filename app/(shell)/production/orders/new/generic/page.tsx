...
async function createOrder(formData: FormData) {
  ...
  if (!parsed.success) {
    redirect(`/production/orders/new/generic?error=${encodeURIComponent(firstErrorMessage(parsed.error))}`);
  }

  const validPriority = parsePriority(priorityRaw, 3);
  if (validPriority === null) {
    redirect(`/production/orders/new/generic?error=${encodeURIComponent("Prioridad invalida (1-5)")}`);
  }
  ...
  if (!warehouse) {
    redirect(`/production/orders/new/generic?error=${encodeURIComponent("Almacen no encontrado")}`);
  }
...
export default async function NewGenericProductionOrderPage({
  searchParams,
}: {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold">Nueva Orden Genérica</h1>
          <p className="text-slate-400 mt-1">Registro manual de orden de producción.</p>
        </div>
