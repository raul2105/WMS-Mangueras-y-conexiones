# Manual de Instalacion WMS Rigentec en Otra Computadora

**Version:** 2026-03-31

## 1. Objetivo
Este manual explica como instalar y operar WMS Rigentec en otra computadora Windows usando el paquete **release portable** (sin instalar Node.js en el equipo destino).
...
## 3. Prerrequisitos
- Windows 10/11 x64
- Acceso a la carpeta del release `wms-rigentec-<version>-windows-x64` (o su `.zip`)
...
- Base de datos SQLite: `%LOCALAPPDATA%\wms-rigentec\data\wms.db`
- Respaldos: `%LOCALAPPDATA%\wms-rigentec\backups\`
- Logs: `%LOCALAPPDATA%\wms-rigentec\logs\`
- Estado/PID: `%LOCALAPPDATA%\wms-rigentec\run\`
...
2. Revisar logs en `%LOCALAPPDATA%\wms-rigentec\logs\`
...
maintenance\backup-db.cmd
```
3. Localizar respaldo mas reciente en `%LOCALAPPDATA%\wms-rigentec\backups\`
...
3. No borrar `%LOCALAPPDATA%\wms-rigentec\`
4. Ejecutar:
```cmd
launcher.cmd
```
