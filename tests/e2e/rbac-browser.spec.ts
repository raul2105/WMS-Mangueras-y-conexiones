...
import { loginAs, expectAllowed, expectRedirectedAllowed, expectForbidden } from "./lib/auth.helpers";
...
test.describe("RBAC en navegador por rol", () => {
  ...
  test("KAN-55/KAN-63/KAN-81 SALES_EXECUTIVE opera el flujo nuevo de pedidos", async ({ page }) => {
    await loginAs(page, "SALES_EXECUTIVE");
    await expectAllowed(page, "/production/requests", /Warehouse Execution Cockpit/i);
  ...
  test("rutas legacy de sales redirigen al flujo nuevo", async ({ page }) => {
    await loginAs(page, "SALES_EXECUTIVE");
    await expectRedirectedAllowed(page, "/sales", /\/production\/requests(?:\?.*)?$/, /Warehouse Execution Cockpit/i);
    await expectRedirectedAllowed(page, "/sales/orders", /\/production\/requests(?:\?.*)?$/, /Warehouse Execution Cockpit/i);
