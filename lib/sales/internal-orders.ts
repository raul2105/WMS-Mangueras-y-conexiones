...
export const SALES_INTERNAL_ORDER_STATUS_LABELS: Record<
  SalesInternalOrderStatus,
  string
> = { BORRADOR: "Borrador", CONFIRMADA: "Confirmada", CANCELADA: "Cancelada" };
export const SALES_INTERNAL_ORDER_STATUS_STYLES: Record<
  SalesInternalOrderStatus,
  string
> = {
...
export type SalesOrderFlowStage =
  ...
  | "entregado"
  | "cancelado";
export const SALES_ORDER_FLOW_STAGE_LABELS: Record<
  SalesOrderFlowStage,
  string
> = {
  captura: "Captura",
...
export type SalesOrderFlowBadgeVariant =
  "neutral" | "accent" | "warning" | "success" | "danger";
export const SALES_ORDER_FLOW_STAGE_BADGE_VARIANTS: Record<
  SalesOrderFlowStage,
  SalesOrderFlowBadgeVariant
> = {
  captura: "neutral",
  por_asignar: "accent",
  en_surtido: "warning",
...
export async function getNextSalesInternalOrderCode(
  db: CodeDb,
  now = new Date(),
) {
  ...
  const lastSequence = lastOrder?.code
    ? Number.parseInt(lastOrder.code.slice(prefix.length), 10)
    : 0;
  const nextSequence = Number.isFinite(lastSequence) ? lastSequence + 1 : 1;
  return `${prefix}${String(nextSequence).padStart(4, "0")}`;
}
export async function getNextGeneratedProductionOrderCode(
  db: CodeDb,
  now = new Date(),
) {
  const year = now.getFullYear();
...
export async function getNextSalesPickListCode(db: CodeDb, now = new Date()) {
  ...
  const lastSequence = lastPickList?.code
    ? Number.parseInt(lastPickList.code.slice(prefix.length), 10)
    : 0;
  const nextSequence = Number.isFinite(lastSequence) ? lastSequence + 1 : 1;
  return `${prefix}${String(nextSequence).padStart(4, "0")}`;
}
...
type TakeOrderEligibility = {
  canTakeOrder: boolean;
  takeBlockedReason: string | null;
  takeActionLabel?: "Tomar pedido" | "Continuar pedido";
};
export function getTakeOrderEligibility(input: TakeOrderEligibilityInput): TakeOrderEligibility {
  if (!input.roles.includes("SALES_EXECUTIVE")) {
    return {
      canTakeOrder: false,
      takeBlockedReason: "Solo un ejecutivo de ventas puede tomar el pedido",
    };
  }
  ...
  if (input.assignedToUserId && !input.pulledAt && input.assignedToCurrentUser) {
    return { canTakeOrder: true, takeBlockedReason: null, takeActionLabel: "Continuar pedido" as const };
  }
  if (input.assignedToUserId) {
    return {
      canTakeOrder: false,
      takeBlockedReason: input.pulledAt ? "El pedido ya está tomado" : "El pedido ya tiene responsable",
    };
  }
  if (!input.isCreatedByManager) {
    return {
      canTakeOrder: false,
      takeBlockedReason: "Solo se pueden tomar pedidos creados por manager",
    };
  }
  return { canTakeOrder: true, takeBlockedReason: null, takeActionLabel: "Tomar pedido" as const };
}
...
export function getMarkDeliveredEligibility(
  input: MarkDeliveredEligibilityInput,
) {
  if (input.deliveredToCustomerAt) {
    return {
      canMarkDelivered: false,
      deliveredBlockedReason: "El pedido ya fue marcado como entregado",
    };
  }
  if (input.status !== "CONFIRMADA") {
    return {
      canMarkDelivered: false,
      deliveredBlockedReason: "El pedido debe estar confirmado",
    };
  }
  ...
  if (!input.hasCompletedConfiguredAssembly) {
    return {
      ...
      deliveredBlockedReason:
        "Todas las órdenes de ensamble ligadas deben estar completadas",
    };
  }
  if (!input.preparedForDeliveryAt) {
    return {
      canMarkDelivered: false,
...
export function getSalesOrderFlowStage(
  input: FlowStageInput,
): SalesOrderFlowStage {
...
export type SalesOrderPrimaryCtaResolutionInput = {
  ...
  latestPickStatus?: string | null;
  /**
   * Exact operational destination for an unfinished configured assembly.
   * When there is more than one pending assembly, callers keep the operator
...
export type SalesOrderPrimaryCtaResolution = {
  ...
  actorRole:
    "SALES_EXECUTIVE" | "WAREHOUSE_OPERATOR" | "MANAGER" | "SYSTEM_ADMIN";
  isPrimary: boolean;
  isAllowed: boolean;
  reason: string;
  blockedReason?: string;
};
export function resolveSalesOrderPrimaryCta(
  input: SalesOrderPrimaryCtaResolutionInput,
): SalesOrderPrimaryCtaResolution {
  const orderHref = `/production/requests/${input.orderId}`;
  const fulfillmentHref = `/production/fulfillment/${input.orderId}`;
  const assemblyHref = input.assemblyHref || `${orderHref}#ensambles`;
  ...
  const actorRole: SalesOrderPrimaryCtaResolution["actorRole"] = isAdmin
    ? "SYSTEM_ADMIN"
    : isManager
      ? "MANAGER"
      : isWarehouse
        ? "WAREHOUSE_OPERATOR"
    : "SALES_EXECUTIVE";
  if (input.flowStage === "entregado" || input.flowStage === "cancelado") {
    return {
      code: "REVIEW_BLOCK",
      action: { label: "Ver historial", href: orderHref },
      actorRole,
      isPrimary: false,
      isAllowed: false,
  ...
  if (input.flowStage === "por_asignar") {
    if (isSales && input.takeEligibility?.canTakeOrder) {
      return {
        ...
        isAllowed: true,
        reason:
          "Pedido confirmado sin asignación; ejecutivo visible puede tomarlo.",
      };
    }
    return {
      code: "REVIEW_BLOCK",
      action: {
        ...
        href: orderHref,
        blockedReason:
          input.takeEligibility?.takeBlockedReason ??
  ...
  if (input.flowStage === "en_surtido") {
    ...
    const needsDirectPick = Boolean(
      input.hasProductLines && input.latestPickStatus !== "COMPLETED",
    );
    if (needsAssembly && (isWarehouse || isManager || isAdmin)) {
      return {
        code: "COMPLETE_ASSEMBLY",
        action: { label: "Completar ensamble", href: assemblyHref },
        actorRole,
        isPrimary: true,
        isAllowed: true,
        reason:
          "Pedido en surtido con líneas configuradas pendientes de ensamble.",
      };
    }
    ...
    const fulfillmentComplete = !needsAssembly && !needsDirectPick;
    if (fulfillmentComplete && (isWarehouse || isManager || isAdmin)) {
      return {
        ...
        isPrimary: true,
        isAllowed: true,
        reason: "Surtido y ensambles completos; el operador debe registrar el área de entrega.",
      };
    }
    return {
      code: "REVIEW_BLOCK",
  ...
  if (input.flowStage === "listo_entrega") {
    if (isSales && input.takeEligibility?.canTakeOrder) {
      return {
        code: "TAKE_ORDER",
        action: { label: input.takeEligibility.takeActionLabel ?? "Tomar pedido", href: orderHref },
        actorRole,
        isPrimary: true,
        isAllowed: true,
        reason: "El responsable asignado debe confirmar que toma el pedido antes de registrar la entrega.",
      };
    }
    if (
      (isSales || isManager || isAdmin) &&
      input.deliveredEligibility?.canMarkDelivered
    ) {
      return {
        code: "MARK_DELIVERED",
    ...
    return {
      ...
      action: {
        ...
        href: orderHref,
        blockedReason:
          input.deliveredEligibility?.deliveredBlockedReason ??
          "Revisar estado operativo del pedido",
      },
      actorRole,
      isPrimary: true,
      isAllowed: false,
      reason:
        "Pedido listo para entrega pero bloqueado por precondición o rol.",
      blockedReason:
        input.deliveredEligibility?.deliveredBlockedReason ??
        "Revisar estado operativo del pedido",
    };
...
export function getSalesOrderFlowNarrative(
  input: SalesOrderFlowNarrativeInput,
): SalesOrderFlowNarrative {
  const flowStage = getSalesOrderFlowStage({
    status: input.status,
    assignedToUserId: input.assignedToUserId,
    preparedForDeliveryAt: input.preparedForDeliveryAt,
  ...
  const primaryCta = resolveSalesOrderPrimaryCta({
    ...
    roles:
      (input as SalesOrderFlowNarrativeInput & { roles?: string[] }).roles ??
      [],
    flowStage,
    hasProductLines: input.hasProductLines,
    hasAssemblyLines: input.hasAssemblyLines,
