...
async function cleanupFixtures() {
  const orderIds = [draftOrderId, confirmedOrderId, overdueOrderId, partialOrderId, receivedOrderId].filter(Boolean);
  ...
  await prisma.supplier.deleteMany({ where: { code: { startsWith: "E2E-JIRA-SUP-" } } });
  await prisma.product.deleteMany({ where: { sku: { startsWith: "E2E-JIRA-SKU-" } } });
}

test.describe.serial("KAN-55/KAN-63/KAN-82/KAN-83/KAN-87/KAN-88/Jira UX audit", () => {
  ...
  test("KAN-55 sales cockpit and role home remain aligned with Jira", async ({ page }) => {
  ...
  test("KAN-87 purchasing list exposes Jira presets and filters the operational queue", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /En tránsito/i })).toBeVisible();
    ...
    await expect(page.getByRole("link", { name: /E2E-PO-OVERDUE-/i }).first()).toBeVisible();

    await page.getByRole("link", { name: /Por recibir hoy/i }).click();
    await expect(page).toHaveURL(/\/purchasing\/orders\?preset=por_recibir_hoy/);
    await expect(page.getByRole("link", { name: /E2E-PO-CONF-/i }).first()).toBeVisible();
    ...
    await expect(page.getByRole("link", { name: /E2E-PO-DRAFT-/i }).first()).toBeVisible();
  });

  test("KAN-88 purchase order detail shows timeline and next action workspace", async ({ page }) => {
    ...
    await expect(page.getByRole("link", { name: /Ver documento oficial/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /Descargar PDF/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /Recibir mercancía/i })).toHaveCount(2);
    await expect(page.getByText(/KAN-88/i)).toBeVisible();
  });

  test("KAN-80 warehouse execution cockpit exposes the new request capture route", async ({ page }) => {
    ...
    await expect(page.getByRole("heading", { name: /Siguiente paso/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /← Pedidos/i })).toBeVisible();
    await expect(page.getByRole("button", { name: /Crear pedido/i })).toBeVisible();
  });

  test("KAN-81 sales console keeps the commercial utility routes understandable", async ({ page }) => {
    await loginAs(page, "SALES_EXECUTIVE", "/production/availability");
    await page.goto("/production/availability");

    await expect(page.getByRole("heading", { name: /Disponibilidad para pedidos/i })).toBeVisible();
    ...
    await expect(page.getByRole("heading", { name: /Equivalencias para pedidos/i })).toBeVisible();
    await expect(page.getByRole("button", { name: /Buscar/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /Limpiar/i })).toBeVisible();
  });

  test("KAN-82 dark/light theme toggle keeps the sales cockpit usable", async ({ page }) => {
    ...
    const themeToggle = page.getByRole("banner").getByRole("button", { name: /Cambiar tema/i });
    await expect(themeToggle).toBeVisible();
    await expect(page.getByRole("link", { name: /\+ Nuevo pedido/i })).toBeVisible();
    await expect(page.locator("html")).toHaveAttribute("data-theme", "light");
  });

  test("KAN-83 purchasing command center keeps the landing and receiving routes clear", async ({ page }) => {
    await loginAs(page, "MANAGER", "/purchasing");
    await page.goto("/purchasing");

    await expect(page.getByRole("heading", { name: /^Compras$/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /Ver ordenes/i })).toBeVisible();
    await expect(page.getByRole("link", { name: /Nueva OC/i })).toBeVisible();

    await page.goto(`/purchasing/orders/${confirmedOrderId}/receive`);
    await expect(page.getByRole("heading", { name: /Recibir Mercancía/i })).toBeVisible();
    await expect(page.getByRole("heading", { name: /Datos de Recepción/i })).toBeVisible();
    await expect(page.getByRole("heading", { name: /Artículos Pendientes/i })).toBeVisible();
  });
});
