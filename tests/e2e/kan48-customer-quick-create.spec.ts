...
function getUser(role: "MANAGER" | "SALES_EXECUTIVE") {
  const email = process.env[`WMS_E2E_${role}_EMAIL`];
  const password = process.env[`WMS_E2E_${role}_PASSWORD`];
  if (!email || !password) {
    throw new Error(`Missing WMS_E2E_${role}_EMAIL/WMS_E2E_${role}_PASSWORD.`);
  }
...
test.describe("KAN-48 customer quick-create", () => {
  ...
  test("sales puede abrir el registro rápido sin acceso a la administración de clientes", async ({ page }) => {
    const user = getUser("SALES_EXECUTIVE");
